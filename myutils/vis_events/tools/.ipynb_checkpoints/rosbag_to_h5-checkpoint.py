...
def read_h5_events(hdf_path, start_idx=None, end_idx=None):
    ''' 
    return numpy array 
    '''
    f = h5py.File(hdf_path, 'r')
    flow, image = [], []
    xs, ys, ts, ps = None, None, None, None 

    if start_idx is None: start_idx = 0
    if end_idx is None: end_idx = -1

    if 'events' in f:
        print("retrive event {} out of {} ".format(end_idx-start_idx,len(f['events']['xs'][:])))
        xs = f['events']['xs'][start_idx:end_idx]
        ys = f['events']['ys'][start_idx:end_idx]
        ts = f['events']['ts'][start_idx:end_idx]
        ps = f['events']['ps'][start_idx:end_idx]

        # hxy_plot_event(f['events']['xs'][:num],f['events']['ys'][:num])
        # hxy_scatter_event(f['events']['xs'][:num],f['events']['ys'][:num],f['events']['ts'][:num])

    if 'images' in f:
        for index, i in enumerate(f['images'].keys()):
            img = f['images'][i][:]
            image.append(img)
            # print("write ", i)
            # cv2.imwrite("extracted_data/extract_images/{}.png".format(i), img)
    ...
    with rosbag.Bag(rosbag_path, 'r') as bag:
        ...
        for topic, msg, t in bag.read_messages(): 
            ...
            if topic == image_topic:
                ...
                event_msg_sum += 1
                # if event_msg_sum < 1300:
                #     continue 
                # if event_msg_sum > 1793:
                #     break    
                if event_msg_sum % num_msgs_between_logs == 0 or event_msg_sum >= num_event_msgs - 1:
                    print('Event messages: {} / {}, contain {} events'.format(event_msg_sum + 1, num_event_msgs,len(msg.events)))
                for e in msg.events:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--path", default="/home/hxt/Desktop/rpg-esim/results/multi_objects/multi_shapes-one.bag",
    ...
    parser.add_argument("--event_topic", default="/cam0/events", help="Event topic")
    parser.add_argument("--image_topic", default="/cam0/image_raw", help="Image topic (if left empty, no images will be collected)")
    parser.add_argument("--flow_topic", default="/cam0/optic_flow", help="Flow topic (if left empty, no flow will be collected)")
    parser.add_argument('--zero_timestamps', default=True, help='If true, timestamps will be offset to start at 0')
    parser.add_argument('--is_color', default=False, help='Set flag to save frames from image_topic as 3-channel, bgr color images')
