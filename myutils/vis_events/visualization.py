...
class Visualization:
    ...
    def return_event_img(self, event_cnt):
        inp_events = event_cnt
        ...
        return (self.events_to_image(inp_events_npy, self.color_scheme)*255).astype(np.uint8)

    def plot_event(self, event_cnt, name):
        inp_events = event_cnt
        height = inp_events.shape[2]
        width = inp_events.shape[3]
        ...
        cv2.imshow(f"{name}", (self.events_to_image(inp_events_npy, self.color_scheme)*255).astype(np.uint8))
        # cv2.waitKey(0)

    @staticmethod
    def viz_events(events: np.ndarray, resolution, name):
        """
        ...
        cv2.namedWindow(f'{name}', cv2.WINDOW_NORMAL)
        cv2.resizeWindow(f'{name}', 400, 400)
        cv2.imshow(f'{name}', image_rgb.astype(np.uint8))
        # cv2.waitKey(0)
        # cv2.destroyAllWindows()

    # ******************************************************************************************************************************
    def plot(self, inputs, flow, name):
        inp_events = inputs["e_cnt"] if "e_cnt" in inputs.keys() else None
        height = inp_events.shape[2]
        width = inp_events.shape[3]
        ...
        inp_events = inp_events.detach()
        inp_events_npy = inp_events.cpu().numpy().transpose(0, 2, 3, 1).reshape((height, width, -1))
        cv2.namedWindow(f"{name} Events", cv2.WINDOW_NORMAL)
        cv2.resizeWindow(f"{name} Events", int(self.px), int(self.px))
        cv2.imshow(f"{name} Events", self.events_to_image(inp_events_npy))
        ...
        if flow is not None:
            ...
            cv2.namedWindow(f"{name} Flow", cv2.WINDOW_NORMAL)
            cv2.resizeWindow(f"{name} Flow", int(self.px), int(self.px))
            cv2.imshow(f"{name} Flow", flow_npy)

    def plot_save(self, inputs, flow, name):
        store_dir = 'trained_models/'

        if "e_cnt" in inputs.keys():
            inp_events = inputs["e_cnt"]
        elif 'inp_cnt' in inputs.keys():
            inp_events = inputs["inp_cnt"]
        else:
            inp_events = None
        ...
        path_to = store_dir + 'results/' + name + "/"
