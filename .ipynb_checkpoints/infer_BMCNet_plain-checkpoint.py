...
from models.BMCNet_plain import BMCNet_plain
...
def infer_body(dataloader_config, data_path, esr_model,
    ...
    for i, inputs_seq in enumerate(tqdm(dataloader, total=len(dataloader))):
        ...
        if init:
            ...
            h, prediction = esr_model(input_stack.to(device), init_h, init_o, init)
            init = False
        else:
            h, prediction = esr_model(input_stack.to(device), h, prediction, init)
...
def load_model(dataloader_config, model_path, device, n_c = 128, n_b = 5):
    ...
    # build ESR
    esr_model = BMCNet_plain(dataloader_config['dataset']['scale'], n_c, n_b)
