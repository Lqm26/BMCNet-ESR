...
from myutils.iwe import purge_unfeasible, get_interpolation, interpolate
...
class EventWarping(nn.Module):
    ...
    def forward(self, flow_list, event_list, pol_mask, resolution):
        """
        ...
        flow_idx[:, :, 0] *= factor[1]  # torch.view is row-major
        ...
        for flow in flow_list:
            ...
            # interpolate forward
            tref = 1
            fw_idx, fw_weights = get_interpolation(event_list, event_flow, tref, factor, self.flow_scaling)

            # per-polarity image of (forward) warped events
            fw_iwe_pos = interpolate(fw_idx.long(), fw_weights, factor, polarity_mask=pol_mask[:, :, 0:1])
            fw_iwe_neg = interpolate(fw_idx.long(), fw_weights, factor, polarity_mask=pol_mask[:, :, 1:2])

            # image of (forward) warped averaged timestamps
            fw_iwe_pos_ts = interpolate(
                fw_idx.long(), fw_weights * ts_list, factor, polarity_mask=pol_mask[:, :, 0:1]
            )
            fw_iwe_neg_ts = interpolate(
                fw_idx.long(), fw_weights * ts_list, factor, polarity_mask=pol_mask[:, :, 1:2]
            )
            ...
            tref = 0
            bw_idx, bw_weights = get_interpolation(event_list, event_flow, tref, factor, self.flow_scaling)

            # per-polarity image of (backward) warped events
            bw_iwe_pos = interpolate(bw_idx.long(), bw_weights, factor, polarity_mask=pol_mask[:, :, 0:1])
            bw_iwe_neg = interpolate(bw_idx.long(), bw_weights, factor, polarity_mask=pol_mask[:, :, 1:2])

            # image of (backward) warped averaged timestamps
            bw_iwe_pos_ts = interpolate(
                bw_idx.long(), bw_weights * (1 - ts_list), factor, polarity_mask=pol_mask[:, :, 0:1]
            )
            bw_iwe_neg_ts = interpolate(
                bw_idx.long(), bw_weights * (1 - ts_list), factor, polarity_mask=pol_mask[:, :, 1:2]
            )
            bw_iwe_pos_ts /= bw_iwe_pos + 1e-9
            bw_iwe_neg_ts /= bw_iwe_neg + 1e-9

            # flow smoothing
            flow = flow.view(flow.shape[0], 2, factor[0], factor[1])
