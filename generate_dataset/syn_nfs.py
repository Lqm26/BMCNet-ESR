...
def get_flags():
    ...
    parser.add_argument('--root_data_path', default='/nfs')
    parser.add_argument('--path_to_h5', default='/nfs/h5_data')
...
if __name__ == '__main__':
    ...
    lllr_dirs = sorted(glob(os.path.join(path_to_lllr, '*.txt')))

    for hr_dir, lr_dir, llr_dir, lllr_dir in zip(hr_dirs, lr_dirs, llr_dirs, lllr_dirs):
        print(f'\n processing {hr_dir, lr_dir, llr_dir, lllr_dir}')
        ...
        ori_lllr_ev = pd.read_csv(lllr_dir, delim_whitespace=True, header=None,
                           names=['t', 'x', 'y', 'pol'],
                           dtype={'t': np.float64, 'x': np.int16, 'y': np.int16, 'pol': np.float64},
                           engine='c',
                           skiprows=1, nrows=None, memory_map=True).values
        lllr_events = transform_events(ori_lllr_ev)
        ...
        ep.package_events('down8', lllr_events[:, 0], lllr_events[:, 1], lllr_events[:, 2], lllr_events[:, 3])
