            ...
            for idx, inputs_seq in enumerate(self.train_dataloader):
                ...
                for inputs in inputs_seq:
                    ...
                    if init:
                        ...
                        h, prediction = self.esr_model(input_stack.to(self.device), init_h, init_o, init)
                        init = False
                    else:
                        h, prediction = self.esr_model(input_stack.to(self.device), h, prediction, init)
...
def main(config_parser):
    ...
    n_c = 128
    n_b = 5
    model = BMCNet_plain(config['train_dataloader']['dataset']['scale'], n_c, n_b)
